...
export default function MyRecipes({ myRecipes, searchBox }: { myRecipes: Recipe[], searchBox: string }) {
    const { t } = useTheme();
                ...
                {myRecipes.length > 0 ? (
                    <div className="px-6">
                        <div className="space-y-4">
                            {myRecipes
                                .filter((recipe) => recipe.title.toLowerCase().includes(searchBox.toLowerCase()))
                                .map((recipe) => (
                                    <div
                                        key={recipe.id}
                    ...
                    <div className="w-full py-16 flex flex-col items-center justify-center">
                        ...
                        <p className="text-gray-700 dark:text-gray-200 font-medium">{t('myList.noMyRecipes')}</p>
                        <p className="text-sm text-gray-500 dark:text-gray-400 max-w-sm text-center mt-2">
                            {t('myList.createRecipeDescription') || "Create your own recipes to share with others"}
                        </p>
                        <Link
