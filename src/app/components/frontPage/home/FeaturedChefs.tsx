    ...
    return (
        <>
            <div className="flex items-center justify-between w-full">
                <h2 className="text-[20px] font-medium dark:text-slate-200">{t('featuredChefs.title')}</h2>
                <div className="flex items-center gap-2 ml-3">
                    <button className="font-semibold cursor-pointer text-[#FF6B35] hover:text-[#e55a29] dark:text-indigo-400 dark:hover:text-indigo-300 transition-colors">{t('featuredChefs.viewAll')}</button>
                    ...
                    <div key={chef.id} className="bg-white dark:bg-gray-800 rounded-[15px] overflow-hidden shadow-sm hover:shadow-md dark:shadow-md dark:shadow-black/30 dark:hover:shadow-lg dark:hover:shadow-black/40 transition-shadow">
                        ...
                        <div className="p-4">
                            ...
                            <div className="flex justify-between mt-4 text-sm dark:text-gray-300">
                                <span>{t('featuredChefs.recipes').replace('{count}', chef.recipes.toString())}</span>
                                <span>{t('featuredChefs.followers').replace('{count}', chef.followers.toString())}</span>
                            </div>
