    ...
    const fetchRecipes = async () => {
        try {
            ...
            if (mainFilterMenu === "Seasonal") {
                ...
                params.append('limit', '1'); // Get just one random recipe
            }
            else {
            ...
            <div id="tableHeaders" className="flex w-full bg-gray-50 dark:bg-gray-700/60 border-b border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 font-medium text-sm">
                {listHeaders.map((header, index) => (
                    <div key={index} className={`
                        ...
                        ${header === "Views" ? "w-0 md:w-1/6 hidden md:flex justify-center font-semibold " + (sortField === "viewcount" ? "text-[#FF6B35] dark:text-indigo-300" : "") : ""}
                        ${header === "Rating" ? "w-0 md:w-1/6 hidden md:flex justify-center font-semibold " + (sortField === "rating" ? "text-[#FF6B35] dark:text-indigo-300" : "") : ""}
                    ...
                    `}>
                        ...
                        {(header === "Views" || header === "Rating") && (
                            <button 
                                ...
                                onClick={(e) => {
                                    e.preventDefault();
                                    handleSort(header === "Views" ? "viewcount" : "rating");
                                }}
                            >
                                {(header === "Views" && sortField === "viewcount") || (header === "Rating" && sortField === "rating") ? (
                                    sortOrder === "asc" ? 
